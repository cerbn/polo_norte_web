...
class NamespaceMap(object):
    ...
    def addAlias(self, namespace_uri, desired_alias, implicit=False):
        ...
        if type(desired_alias) in [str]:
            assert '.' not in desired_alias, "%r must not contain a dot" % (desired_alias,)
        ...
        assert (desired_alias == NULL_NAMESPACE or
                type(desired_alias) in [str]), repr(desired_alias)  # Removido unicode
